...
import type { SymbolSet, PlanSymbolOverrides } from '@/types/symbol-set';
import { resolveSymbol } from '@/lib/symbols/resolve';
...
interface PlantPlacement {
  ...
  /** When the plant was generated by the Voronoi tapestry layout, this is its
   *  cell polygon. Renderers use it as the tapestry shape instead of the
   *  procedural blob, giving a true gap-free Oudolf tessellation. */
  cellGeoJson?: GeoJSON.Polygon;
  /** Botanical family + hierarchy tier — used by the symbol resolver to pick
...
interface MapboxMapProps {
  ...
  /** Active symbol set; when present and `showSymbols` is true, an Oudolf-
   *  style glyph is drawn at each plant's centroid. */
  symbolSet?: SymbolSet | null;
  /** Per-plan symbol overrides (rides on the plan doc, doesn't mutate the
   *  shared set). */
  planSymbolOverrides?: PlanSymbolOverrides;
...
function buildTapestryGeoJSON(placements: PlantPlacement[]): GeoJSON.FeatureCollection {
  return {
    ...
    features: placements.map((p, i) => {
      ...
      // tiles the bed without voids. Use it directly.
      if (p.cellGeoJson && p.cellGeoJson.coordinates?.[0]?.length >= 4) {
        return {
          type: 'Feature' as const,
      ...
      // Fallback: procedural blob noise around the centroid (pre-Voronoi
      // plans, or layouts that don't ship cells).
...
// ---------------------------------------------------------------------------
// Symbol-layer support — Piet Oudolf-style glyphs rendered over the plant
// placements. The renderer follows Oudolf's plan-drawing convention:
//
//   • A *drift* (≥2 cells of the same species) is rendered as a tiled
//     PATTERN over the merged cell footprint, in the species' palette
//     color. This reads like a hand-drawn drift annotation: dozens of
//     repeating asterisks across an Echinacea sweep, a hatched grass
//     pattern across a matrix planting, etc.
//   • A *solitary* plant (exactly 1 cell of that species in the plan) is
//     drawn as a single centered icon — the convention for trees and
//     specimen plantings.
//   • Both layers run at slight opacity so the underlying tapestry color
//     shows through.
//
// Color comes from the species' OUDOLF_PALETTE entry (same hue used for
// the tapestry blob fill), not the symbol set's default — so the symbol
// reads as part of the species' visual identity, not as a black overlay.
// ---------------------------------------------------------------------------

const SYMBOL_TILE_SIZE = 56; // pattern tile in screen pixels
const SYMBOL_SOLO_SIZE = 80; // solo glyph image size

/** Per-tier scaling of the inner glyph relative to the pattern tile.
 *  Pattern fill in Mapbox tiles at the image's natural pixel size in
 *  screen space, NOT in cell coordinates. So small cells (matrix at 17px
 *  wide on screen) need a much smaller tile than emergent cells, or the
 *  pattern is too sparse to read.
 *  Returned as (tileSize, glyphSize) in pixels. */
function tierTileMetrics(tier: number | undefined): { tile: number; glyph: number } {
  switch (tier) {
    case 5: return { tile: 56, glyph: 40 }; // emergent — 1–2 glyphs per cell
    case 4: return { tile: 40, glyph: 28 };
    case 3: return { tile: 28, glyph: 20 };
    case 2: return { tile: 22, glyph: 14 }; // matrix — continuous fine texture
    case 1: return { tile: 18, glyph: 11 };
    default: return { tile: 28, glyph: 20 };
  }
}

/** Darken a hex color by mixing toward black. Used to give the symbol a
 *  shade darker than its species cell so it reads as ink on watercolor
 *  rather than disappearing into the underlying blob. */
...
/** Rasterize an SVG-body string into an HTMLImageElement. Used for both
 *  the solo icon (full-bleed glyph on transparent bg) and the pattern tile
 *  (smaller glyph centered on transparent padding so tiling reads as
 *  "dots in space" rather than a wall of colour). */
function svgToImage(
  innerSvg: string,
  color: string,
  outerSize: number,
  ...
  // We embed two SVGs: outer wrapper at outerSize, inner glyph drawn at
  // innerSize, centred. Inner uses 24×24 viewBox (the symbol-set convention).
  const offset = (outerSize - innerSize) / 2;
  const wrapped = `<svg xmlns="http://www.w3.org/2000/svg" width="${outerSize}" height="${outerSize}" viewBox="0 0 ${outerSize} ${outerSize}">` +
    `<svg x="${offset}" y="${offset}" width="${innerSize}" height="${innerSize}" viewBox="0 0 24 24" style="color:${color}">${innerSvg}</svg>` +
    `</svg>`;
  const dataUri = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(wrapped)}`;
  return new Promise((resolve, reject) => {
    const img = new Image(outerSize, outerSize);
...
/** Build/refresh the symbol layers (pattern fill for drifts, solo icon for
 *  singletons). Idempotent across re-renders — uses `map.hasImage()` to
 *  skip already-registered sprites. */
async function addSymbolLayer(
  map: mapboxgl.Map,
  plants: PlantPlacement[],
  symbolSet: SymbolSet,
  planOverrides: PlanSymbolOverrides | undefined,
  visible: boolean,
) {
  // ---- Group placements by species ------------------------------------
  const bySpecies = new Map<string, PlantPlacement[]>();
  for (const p of plants) {
    if (!p.lat || !p.lng) continue;
    const list = bySpecies.get(p.slug);
    if (list) list.push(p);
    else bySpecies.set(p.slug, [p]);
  }

  const driftFeatures: GeoJSON.Feature[] = [];
  const soloFeatures: GeoJSON.Feature[] = [];
  const seenImages = new Set<string>();
  const iconLoads: Promise<unknown>[] = [];

  for (const [slug, group] of bySpecies) {
    const first = group[0];
    const resolved = resolveSymbol(
      { slug, family: first.family ?? '', tier: first.tier, plantType: first.plantType as never },
      symbolSet,
      planOverrides,
    );
    // The symbol "ink" is a darkened version of the species' tapestry hue.
    // Same hue as the blob fill (so it reads as part of that species'
    // identity) but lower lightness — visible against the blob underneath
    // like ink stippling on a watercolor.
    const speciesColor = darkenHex(tapestryColor(first.speciesIndex || 0, slug));
    const colorTag = speciesColor.replace(/[^a-z0-9]/gi, '');
    const tier = first.tier ?? 3;
    const { tile, glyph } = tierTileMetrics(tier);
    const patternKey = `pat-${slug}-${colorTag}-${tier}`;
    const iconKey = `ico-${slug}-${colorTag}`;

    // Register pattern tile (transparent-bg glyph for fill-pattern).
    if (!seenImages.has(patternKey) && !map.hasImage(patternKey)) {
      seenImages.add(patternKey);
      iconLoads.push(
        svgToImage(resolved.svg, speciesColor, tile, glyph)
          .then((img) => { if (!map.hasImage(patternKey)) map.addImage(patternKey, img); })
          .catch(() => {/* swallow */}),
      );
    }
    // Register solo icon (single-glyph image, larger so a lone plant reads).
    if (!seenImages.has(iconKey) && !map.hasImage(iconKey)) {
      seenImages.add(iconKey);
      iconLoads.push(
        svgToImage(resolved.svg, speciesColor, SYMBOL_SOLO_SIZE, SYMBOL_SOLO_SIZE - 12)
          .then((img) => { if (!map.hasImage(iconKey)) map.addImage(iconKey, img); })
          .catch(() => {/* swallow */}),
      );
    }

    if (group.length >= 2) {
      // Drift: emit a single MultiPolygon collecting every cell. We don't
      // dissolve shared edges — Mapbox tiles the pattern across whatever
      // we hand it, so an unmerged collection draws identically to a
      // dissolved one and avoids a turf.union over potentially 50+ cells.
      const polys: GeoJSON.Polygon['coordinates'][] = [];
      for (const p of group) {
        if (p.cellGeoJson?.coordinates) polys.push(p.cellGeoJson.coordinates);
      }
      if (polys.length === 0) {
        // No cell polygons (legacy plan) — fall back to soloing each cell.
        for (const p of group) {
          soloFeatures.push(soloFeatureFor(p, iconKey, resolved.scale));
        }
        continue;
      }
      driftFeatures.push({
        ...
        properties: { slug, patternKey, color: speciesColor },
      });
    } else {
      soloFeatures.push(soloFeatureFor(first, iconKey, resolved.scale));
    }
  }

  await Promise.all(iconLoads);

  const driftFc: GeoJSON.FeatureCollection = { type: 'FeatureCollection', features: driftFeatures };
  const soloFc: GeoJSON.FeatureCollection = { type: 'FeatureCollection', features: soloFeatures };

  // ---- Pattern-fill layer (drifts) ------------------------------------
  const driftSrc = map.getSource('plant-symbol-drifts') as mapboxgl.GeoJSONSource | undefined;
  if (driftSrc) {
    driftSrc.setData(driftFc);
  } else {
    map.addSource('plant-symbol-drifts', { type: 'geojson', data: driftFc });
    map.addLayer({
      id: 'plant-symbol-drifts',
      type: 'fill',
      source: 'plant-symbol-drifts',
  ...
  // ---- Solo-icon layer (singletons) -----------------------------------
  const soloSrc = map.getSource('plant-symbols') as mapboxgl.GeoJSONSource | undefined;
  if (soloSrc) {
    soloSrc.setData(soloFc);
  } else {
    map.addSource('plant-symbols', { type: 'geojson', data: soloFc });
    map.addLayer({
      id: 'plant-symbols',
      type: 'symbol',
      source: 'plant-symbols',
      layout: {
        ...
        'icon-size': [
          ...
          ['linear'],
          ['zoom'],
          17, ['*', 0.16, ['coalesce', ['get', 'scale'], 1], ['coalesce', ['get', 'tierSize'], 0.8]],
          19, ['*', 0.30, ['coalesce', ['get', 'scale'], 1], ['coalesce', ['get', 'tierSize'], 0.8]],
          21, ['*', 0.55, ['coalesce', ['get', 'scale'], 1], ['coalesce', ['get', 'tierSize'], 0.8]],
        ],
...
function soloFeatureFor(
  p: PlantPlacement,
  iconKey: string,
  scale: number,
): GeoJSON.Feature {
  // Cell centroid when the layout produced one; otherwise the lat/lng.
  let cx = p.lng;
  let cy = p.lat;
  if (p.cellGeoJson?.coordinates?.[0]?.length) {
    let sx = 0, sy = 0, n = 0;
    for (const c of p.cellGeoJson.coordinates[0]) { sx += c[0]; sy += c[1]; n++; }
    cx = sx / n;
    cy = sy / n;
  }
  return {
...
export default function MapboxMap({
  ...
  const planSymbolOverridesRef = useRef(planSymbolOverrides);
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // Symbol layer — Oudolf-style glyphs from the active set. Async because
      // each unique glyph needs to round-trip through SVG → image decode →
      // map.addImage before the symbol layer can reference it.
      if (symbolSetRef.current) {
        addSymbolLayer(
          map,
          plantPlacementsRef.current,
          symbolSetRef.current,
          planSymbolOverridesRef.current,
          showSymbolsRef.current,
        ).catch(() => {/* swallow — keeps tapestry/numbered layers alive */});
      }
    ...
    mapRef.current = map;
    // Dev-only: expose for in-browser debugging via window.__planMap.
    if (process.env.NODE_ENV !== 'production') {
      (window as unknown as { __planMap?: mapboxgl.Map }).__planMap = map;
    }
  ...
  useEffect(() => {
    ...
    const update = () => {
      try {
        ...
        // Re-resolve and re-paint symbols whenever the plant list, set, or
        // overrides change. addSymbolLayer is idempotent: it updates the
        // existing source and skips already-loaded sprite images.
        if (symbolSetRef.current) {
          addSymbolLayer(
            map,
            plantPlacements,
  ...
  }, [plantPlacements, symbolSet, planSymbolOverrides, showSymbols]);

  // Toggle symbol-layer visibility when showSymbols flips without a data
  // change. Cheaper than re-running addSymbolLayer. Both the drift pattern
  // layer and the solo-icon layer flip together.
  useEffect(() => {
    const map = mapRef.current;
    if (!map) return;
    const vis = showSymbols ? 'visible' : 'none';
    if (map.getLayer('plant-symbols')) map.setLayoutProperty('plant-symbols', 'visibility', vis);
    if (map.getLayer('plant-symbol-drifts')) map.setLayoutProperty('plant-symbol-drifts', 'visibility', vis);
  }, [showSymbols]);
